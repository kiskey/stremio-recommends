...
MANIFEST = {
    ...
    "version": "4.1.0", # Version reflects the new, robust entrypoint logic
    ...
    "types": ["movie", "series"],
    "resources": ["catalog"], # We do not handle 'meta' resources
    "catalogs": [
...
def generate_sorted_recommendations(media_type: str, skip: int = 0):
    ...
    try:
        ...
        if history_df.empty:
            print(f"[main.py] No watch history found for type '{media_type}'. Returning empty catalog.")
            conn.close()
...
def create_app():
    ...
    # The pre-flight check for artifact files is now handled by entrypoint.sh.
    # We can proceed directly to loading the data.
    ...
    # Initialize history DB (create the file and table if they don't exist)
    try:
